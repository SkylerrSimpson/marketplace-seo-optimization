 ...
 * for Ethan (eBay_{ACCT}_current-live-attributes_REVIEW_v3.csv). A fresh,
 ...
 * (confirmed 2026-07-09 spot-checking the UPC/Country-Region-of-Manufacture
...
foreach ($ids as $itemId) {
    ...
    if ($snap === null) {
        $errors++;
        if ($done % 25 === 0) { fwrite(STDOUT, "  {$done}/" . count($ids) . " (errors {$errors}, rows {$rows})\n"); }
        continue;
    }
