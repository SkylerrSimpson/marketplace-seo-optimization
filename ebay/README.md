...
| — | `apply_review_rules.php` | Applies Ethan's 4 proposing rules (allowed-value snap, Country default, Mfr Warranty, blank_value) into `proposed_value`. Re-run **after** `build_review_sheet.php` or it gets clobbered. Prop 65 (former rule #2) is removed as of 2026-07 — see below. |
...
| — | `delete_prop65_live.php` | 2026-07 Prop65 removal, step 2 — the actual live write. Deliberately does NOT read `approved_value`/`apply_set.json` (avoids sweeping in other unrelated pending approvals); rebuilds each item's specifics from `items/{id}.json`'s own current state, Prop65 key removed. Same `VerifyOnly`/`--live`/`--confirm=WRITE` model. Logs to `delete_prop65_run.csv`. **Bulk runs are held until the new description badge (see below) is actually live for that account.** |
...
| 8 | `apply_descriptions.php` | **Stage 2, live write.** Pushes `description_review.csv`'s `new_html`/`new_title` to eBay via `ReviseItem` — only rows where `approved` is a truthy marker (yes/y/true/1/approved/approve, Ethan's decision column). Same `VerifyOnly`-default / `--live` + `--confirm=WRITE` model as `apply_aspects.php`. No re-authoring, no LLM calls — purely a transport for what's already in the sheet. Logs to `apply_descriptions_run.csv`. |
