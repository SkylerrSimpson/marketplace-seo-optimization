# Current-value verification — handoff & merge guide (for Skyler)
...
We're delegating the judging to external agents to save Claude Code usage.
...
3. Save its raw output to `ebay/handoff/returned/<slice-name>.out.jsonl`
...
comm -3 \
  <(grep -o '"item_id":"[0-9]*"' ebay/handoff/slices/dows_slice_00.jsonl | sort -u) \
  <(grep -o '"item_id":"[0-9]*"' ebay/handoff/returned/dows_slice_00.out.jsonl | sort -u)
```
