...
DOWScripts is a Laravel web UI in front of the existing marketplace CLI scripts that
live in the parent repo (`../ebay/scripts/`, `../shopify/scripts/`, ...). Read
`../dowscripts/PLAN.md` first for the full design — this file is the short version:
the rules that must hold on every change, not the reasoning behind them.
